...
> 상태: **Discord 사용 가능**. 코어·브리지·영속화·락·Discord 게이트웨이·이미지 전달·진행 카드·작업 다이제스트가 416개 테스트로 고정되어 있다. 봇 SDK(`discord.js` 등) 없이 구현했다.
...
> 첨부 다운로드·락은 전부 가짜 소켓과 가짜 fetch로 테스트되어 있고(416개), 첫 실사용이
...
긴 응답의 분할 상한은 `bridge.maxChunks`(기본 8, 최대 50)로 조절한다. Discord 한도(2000자)를 넘는 응답은 여러 메시지로 나뉘고, 상한을 넘으면 잘렸다고 알린다.
